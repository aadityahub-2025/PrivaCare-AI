...
║         Model: Random Forest  |  Mechanism: Laplace (pure ε-DP)             ║
║                                                                              ║
║  DP Correctness:                                                             ║
║  · Data-independent normalization (domain bounds, not data min/max)         ║
║  · Laplace scale b = sensitivity/epsilon = 1.0/epsilon                      ║
║  · Pure epsilon-DP guarantee (no delta needed)                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
# Laplace mechanism -- no diffprivlib needed (pure numpy)
...
def load_and_train(epsilon=0.5):
    ...
    # DP model — Laplace Mechanism
    # b = sensitivity / epsilon = 1.0 / epsilon  (sensitivity=1.0 from [0,1] normalization)
    b   = 1.0 / epsilon
    ...
    for i, col in enumerate(feature_cols):
        if col not in BINARY_FEATURES:
            noise = rng.laplace(loc=0.0, scale=b, size=X_train_norm.shape[0])
